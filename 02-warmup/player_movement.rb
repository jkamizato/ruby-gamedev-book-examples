...
class Tank
  ...
  def update
    ...
    @body_angle = change_angle(@body_angle, Gosu::KbW, Gosu::KbS, Gosu::KbA, Gosu::KbD)
  end

  def draw
    @shadow.draw_rot(@x - 1, @y - 1, 0, @body_angle)
    @body.draw_rot(@x, @y, 1,  @body_angle)
    @gun.draw_rot(@x, @y, 2, @body_angle)
  end
  ...
  def change_angle(previous_angle, up, down, right, left)
    if @window.button_down?(up)
      ...
      angle _= 45.0 if @window.button_down?(down)
...
class GameWindow < Gosu::Window
  ...
  UNIT_FILE = File.join(File.dirname(File.dirname(__FILE__)), '..', 'media', 'ground_units.json')
  SPEED = 5
